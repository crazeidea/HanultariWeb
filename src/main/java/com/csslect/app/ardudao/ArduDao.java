...
public class ArduDao {
	...
	public void arduSetIn(int idin, int state) {
		 ...
		 try { 
			 ...
			 if(state == 0) {
				 query = "insert into parkinglog(id, enter_time) " + 
						 "values(?, sysdate)";
			 }else if(state == 1) {
				 query = "update parkinglog set " +
						 "exit_time = sysdate " +
						 "where id = ? " +
			 ...
			 prepareStatement.setInt(1, idin);  
	 ...
	 * 
	 * ArrayList<ArduDto> adtos = new ArrayList<ArduDto>(); Connection connection =
	 ...
	 * "select * from arduino" + " where (id, updatetime)" +
	 ...
	 * = resultSet.getString("name"); String value = resultSet.getString("value");
	 * String updatetime = resultSet.getString("updatetime");
	 * 
	 * ArduDto adto = new ArduDto(id, name, value, updatetime); adtos.add(adto); }
	 * 
	 ...
	 * 
	 * if(id.equals("CSSLed01")) { name = "LivingRoomLight"; }else
	 * if(id.equals("CSSSec01")) { name = "Secom"; }
	 * 
	 * int state = -1;
	 * 
	 * ArrayList<ArduDto> adtos = new ArrayList<ArduDto>(); Connection connection =
	 ...
	 * 
	 * try { connection = dataSource.getConnection(); String query =
	 * "insert into arduino(id, name, value, updatetime)" +
	 * "values(?, ?,?, to_char(sysdate, 'yy/mm/dd hh24:mi:ss') )";
	 * 
	 * prepareStatement = connection.prepareStatement(query);
	 * prepareStatement.setString(1, id); prepareStatement.setString(2, name);
	 * prepareStatement.setString(3, value); state =
	 * prepareStatement.executeUpdate();
	 * 
